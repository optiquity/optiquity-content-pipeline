...
# readme — Format entry (framework default)
...
3. **Getting started** — the shortest honest path to a first result (prerequisites,
   install, first run) — every command grounded in the source (EXTRACTED-tier facts
   only, §6.5; never invented flags).
4. **Usage** — the two or three most-wanted tasks, each with a worked example.
5. **Pointers** — where deeper docs, license, and contribution notes live.

Discipline: heavily grounded — a readme's factual claims (commands, flags, paths,
