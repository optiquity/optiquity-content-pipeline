...
At the product level the pipeline works in two stages. **Ideation** reads a source repo's knowledge graph together with your target audiences and proposes a ranked queue of ideas worth writing. **Generation** turns one chosen idea into a finished, formatted artifact.
...
- **Ideation is not built yet.** The system generates content *for* a topic across the whole matrix, but it cannot yet discover or rank *what* to write about — you hand-author each topic. That is half of the intended two-stage design.
- **The invocation transport is unverified.** Subscription-auth headless operation and the n8n→headless mechanism are gated before build, not yet proven.
- **The HTTP shim has honest edges.** Composing in a single HTTP call is not wired (the supported path is two calls); the concurrency cap is advisory; and one DNS-rebinding edge is mitigated, not fully closed.
- **Several features are reserved for later** — language/localization fanout, per-part dimension overrides, and others — each designed to slot in additively.
