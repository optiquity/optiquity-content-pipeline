...
- **H2 lifecycle:** happy `create_exclusive` → one record, spawn; a re-submit within grace or with
  a live claim → existing, NO re-spawn; a runner-died-pre-claim (past grace, no claim, output
  absent) → STEAL + re-spawn.
- **H3 truth-table (all 5 branches):** DONE (output) → RUNNING (live claim) → RUNNING (startup
  grace / nascent-None) → FAILED (stored terminal) → FAILED-re-drivable. An EXPIRED-lease `peek`
  is NOT live (the resolver compares `lease_expiry` to `now` itself).
- **§22.7 lossy discipline:** a present record with the output absent is NEVER DONE; a DELETED
  ...
  claim / past grace resolves re-drivable — never a wedge, never a false DONE.
- **N-5:** a deterministic block stores NOTHING (the sweep re-derives it); a nondeterministic
  failure round-trips the terminal envelope and surfaces at resolver step 4.
...
class TestSubmitLifecycle:
    ...
    def test_a_one_shot_iterator_input_is_materialized_once(self, store, auth, jobs_dir):
        ...
        again = store.submit(
            (x for x in (A, B)), "idk-1", now=T0 + GRACE + 1, is_done=auth.is_done, peek=auth.peek
        )
        assert again.disposition == "done"
        assert again.spawn is False

    def test_a_resubmit_within_grace_does_not_respawn(self, store, auth):
        first = self._submit(store, auth, now=T0)
        second = self._submit(store, auth, now=T0 + GRACE / 2)  # still within the startup grace
    ...
    def test_a_live_claim_past_grace_does_not_respawn(self, store, auth):
        # Past the startup grace, but a runner holds a LIVE claim on a target → actively running,
        # so the re-submit returns the existing job (no re-spawn).
        self._submit(store, auth, now=T0)
        auth.claims[A] = T0 + 10_000  # a live lease (now < lease_expiry)
        out = self._submit(store, auth, now=T0 + GRACE + 1)
        assert out.spawn is False
        assert out.disposition == "existing"

    def test_runner_died_pre_claim_past_grace_steals_and_respawns(self, store, auth, jobs_dir):
        # THE H2 case: submit → runner dies BEFORE acquiring any claim → past grace, no live claim,
        # output absent → a re-submit STEALS the stale record and re-spawns (not a FileExistsError
        # wedge, and not a second spawn while still within grace — see the two guards above).
        first = self._submit(store, auth, now=T0)
        out = self._submit(store, auth, now=T0 + GRACE + 1)
        assert out.spawn is True
        assert out.disposition == "stolen"
        assert out.key == first.key
        # The steal reset the grace: the record's spawn_time is now the re-submit's clock.
        record = json.loads((jobs_dir / out.key).read_bytes())
        assert record["spawn_time"] == T0 + GRACE + 1
    ...
    def test_a_partially_materialized_job_past_grace_still_steals(self, store, auth):
        # Only SOME targets done, no live claim, past grace → the job is not complete and the
...
class TestSubmitNascentWindow:
    """The §22.3 nascent-write window for the record itself: a torn record within an implicit
    mtime grace is NOT stolen (that would double-spawn a live job); past it, it is stealable."""
    ...
    def test_a_torn_record_within_mtime_grace_is_not_respawned(self, store, auth):
    ...
    def test_a_torn_record_past_mtime_grace_is_stolen(self, store, auth):
        key = self._torn_record(store)
        os.utime(store.path_for(key), (T0, T0))
        out = store.submit(
            (A, B), "idk-1", now=T0 + GRACE + 1, is_done=auth.is_done, peek=auth.peek
        )
        assert out.spawn is True
        assert out.disposition == "stolen"
...
# ---------------------------------------------------------------------------
# (c-H3) resolve_job_state: the corrected truth-table, all 5 branches.
# ---------------------------------------------------------------------------
...
class TestResolverTruthTable:
    ...
    def test_branch2_live_claim_is_running(self, auth):
        auth.claims[A] = T0 + 500  # live lease
        # Past grace so ONLY the live claim can produce RUNNING (isolates branch 2).
        state = _resolve(_record(spawn_time=T0), auth, now=T0 + GRACE + 1)
        assert state == JobState("running", "claim-live")

    def test_branch3_startup_grace_nascent_none_is_running(self, auth):
        # A nascent-None claim (peek None) WITHIN the startup grace → RUNNING (falls to the record
        # branch, NOT FAILED). The output is absent and there is no claim yet.
        state = _resolve(_record(spawn_time=T0), auth, now=T0 + GRACE / 2)
        assert state == JobState("running", "startup-grace")

    def test_branch4_stored_terminal_is_failed_with_reason(self, auth):
        terminal = JobTerminal(code="api-error", envelope={"ok": False}, results=[])
        # Past grace, no claim, output absent → the stored terminal surfaces (branch 4).
        state = _resolve(_record(spawn_time=T0, terminal=terminal), auth, now=T0 + GRACE + 1)
        assert state.kind == "failed"
        assert state.detail == "stored-terminal"
        assert state.terminal == terminal

    def test_branch5_no_live_work_is_redrivable(self, auth):
        # Record present, past grace, no claim, no terminal, output absent → re-drivable.
        state = _resolve(_record(spawn_time=T0), auth, now=T0 + GRACE + 1)
...
class TestResolverExpiredLease:
    def test_an_expired_lease_peek_is_not_running(self, auth):
        ...
        # to now itself. Past grace so branch 3 cannot mask it: an EXPIRED lease → NOT running.
...
class TestLossyDiscipline:
    def test_record_present_output_absent_is_never_done(self, auth):
        # A present record with the output absent is NEVER DONE — whether within grace (running) or
        # past it (re-drivable), the kind is never "done".
        within = _resolve(_record(spawn_time=T0), auth, now=T0 + GRACE / 2)
        past = _resolve(_record(spawn_time=T0), auth, now=T0 + GRACE + 1)
    ...
    def test_deleted_record_output_present_is_done(self, auth):
        ...
        auth.done.add(A)
        state = _resolve(None, auth, now=T0 + 10 * GRACE)
        assert state == JobState("done", "output-exists")

    def test_deleted_record_no_output_no_claim_past_grace_is_redrivable(self, auth):
        # A deleted record + output absent + no claim + past grace → re-drivable (no wedge, no
        # false DONE). The record branch (3/4) cannot fire with record=None.
        state = _resolve(None, auth, now=T0 + 10 * GRACE)
        assert state == JobState("failed-redrivable", "no-live-work")

    def test_store_resolve_reads_a_real_record_and_an_unreadable_one_is_absent(self, store, auth):
        # JobStore.resolve loads by key. A live record within grace → running; a torn record
        ...
        running = store.resolve(
            out.key, A, now=T0 + GRACE / 2, is_done=auth.is_done, peek=auth.peek
        )
        assert running == JobState("running", "startup-grace")
        store.path_for(out.key).write_bytes(b"garbage")  # torn record
        assert store.load(out.key) is None
        absent = store.resolve(out.key, A, now=T0 + GRACE + 1, is_done=auth.is_done, peek=auth.peek)
...
class TestRecordTerminal:
    ...
    def test_a_deterministic_block_stores_nothing(self, store, auth):
        ...
        # And a poll past grace with no claim resolves re-drivable (branch 5), not FAILED.
        state = store.resolve(out.key, A, now=T0 + GRACE + 1, is_done=auth.is_done, peek=auth.peek)
        assert state == JobState("failed-redrivable", "no-live-work")
    ...
    def test_a_nondeterministic_failure_round_trips_and_surfaces(self, store, auth):
        ...
        # A poll past grace with no live claim surfaces the stored terminal (branch 4).
        state = store.resolve(out.key, A, now=T0 + GRACE + 1, is_done=auth.is_done, peek=auth.peek)
        assert state.kind == "failed"
        assert state.detail == "stored-terminal"
    ...
    def test_a_steal_clears_a_previously_stored_terminal(self, store, auth):
        # A re-submit that steals a stale record writes a FRESH running record — the old terminal
        # is gone (the job is genuinely re-driven).
        out = store.submit((A, B), "idk-1", now=T0, is_done=auth.is_done, peek=auth.peek)
