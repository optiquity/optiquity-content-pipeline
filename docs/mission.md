...
> **Status:** Living document · **Version:** 0.2.6 · **Last updated:** 2026-07-24
...
| Version | Date | Change |
|---|---|---|
| 0.3.3 | 2026-08-11 | **TRANSPORT SELECTION** BUILT — an **API-KEY transport** now runs ALONGSIDE the subscription, money-safe by construction (G0–G10, `8200346`..`f2f6b76`, each coder→reviewer→commit; G0 verify + G10 docs spend nothing). **Two transports:** **SUBSCRIPTION** (headless OAuth) is now used ONLY for the SINGLE config-entitled user (ToS), behind a fail-closed **wall** — the widened env-strip (`ANTHROPIC_API_KEY` + `ANTHROPIC_AUTH_TOKEN` + `CLAUDE_CODE_USE_BEDROCK`/`_VERTEX`/`_FOUNDRY`) AND a controlled-settings file proven to define no `apiKeyHelper`, refusing to spawn when the wall can't be proven; and **API-KEY** (single- or multi-tenant), opt-in by **explicit per-scope key assignment**. **F10 strengthened, not removed:** `ANTHROPIC_API_KEY` still never rides a subscription spawn; an api-key spawn is bounded instead. **Keystore (two-layer, shared with sources):** non-secret handles `<ns>:<name>` in gitignored `instance/ops/transport/config.yaml`; secret VALUES in a 0600-per-handle store under `$OPTIQUITY_SECRETS_DIR` (default `~/.optiquity/secrets/`), fail-closed on a relative/in-repo dir or loosened perms, the value never in argv/log/ledger/telemetry (I5). **Assignments + cascade:** `transport assign-key --scope <global\|user:<u>\|zone:<u>/<z>\|workspace:<u>/<z>/<w>> --handle <ns:name> --weekly-cap $C` (cap HARD-required — no silent default, no uncapped key); cascade **most-specific-wins workspace → zone → user → global**, scope-id PARSED from `--scope` (never derived from a `users/…` path, M3), same-named zones under different users DISTINCT. **Entitlement:** `transport set-subscription-user <u>` — exactly ONE, swappable, config-only (never a run param, I2/I3). **Weekly meter + umbrella:** an enforcing per-bucket weekly cap + one install umbrella (default **$50/week** via `set-umbrella-cap`); calendar week Mon 00:00 UTC (injectable clock); a hash-chained **edit-EVIDENT** ledger (NOT reset-proof — a local `rm` reopens, R2); TTL holds; an **INTER-PROCESS** admission lock; **admit-before-spend** (reserves the worst-case ceiling for the bucket AND umbrella in one critical section, refuses pre-spend if either would be exceeded) + **settle-in-`finally`**. **Chokepoint (covers CLI + served HTTP):** enforcement lives at the shared tiers the CLI and the detached `jobrunner→invoke()` re-entry both traverse — resolve→admit→drive→settle at `invoke()`/the session handler, and at `build_child_env` the key injects + strip disarms ONLY under a **live, injected-clock-checked hold** (absent/expired → fail-closed, no spawn); every api-key call forced under `--max-budget-usd = min(caller, $5)` (the **$5 default per-call cap**; subscription stays flat-rate `per_call_cap=None`, never truncated). Per-run `--transport subscription\|api\|key:<handle>` override (a CLOSED vocabulary — a mode or handle, NEVER a user). The **multi-zone S4 "name a zone or refuse"** guard now fires UNIFORMLY at the chokepoint (CLI + served `generate-next`/`render`), closing the zone restructure's CLI-only gap. **Disclosure:** a pre-spend line (mode + bucket + `≤ $C` ceiling + `cap − week-to-date` headroom) to stderr; dry-run discloses + spends nothing. **Zone rung NOW DELIVERED:** per-zone keys/caps + spend-bucket isolation between same-named zones (closes 0.3.2's deferred transport-build item). **Identity-neutral:** transport/mode/handle/zone enter no id preimage — recorded as provenance only. **Residual risks (honest, R1–R6):** R1 `--user` is addressing not auth (bounded blast radius via caps); R2 a local `rm` reopens the cap (operator-bounds-own-bill); R3 single writer host (clock skew / non-locking NFS out of scope); R4 hand-forged in-process plan out of scope; R5 the key is in the child env, visible via `/proc` to the same UID; R6 an assigned OAuth token on a metered plan under-counts. **Operator note:** a host shipping an enterprise/managed `apiKeyHelper` (which `--setting-sources` cannot exclude) makes EVERY subscription spawn refuse (correct, loud, fail-closed) — run on an api key there. Design homes: design §21.9 (F10 strengthened), NEW §21.10 (the single home), §3.3 (keystore two-layer + meter no-secret), §23 (the `instance/ops/{transport,spend}/` stores + explicit-scope credential cascade). NEW `docs/transport.md` (operator guide). Docs swept: design §3.3/§10/§21.9/§21.10/§23, `docs/{transport,bootstrap,claude-code-usage}.md`, `docs/guide/{interfaces,clients,concepts}.md`, `README.md`, `quickstart.md`. |
| 0.3.2 | 2026-08-10 | **ZONE RESTRUCTURE** BUILT — a **zone** now sits between user and workspace, so a workspace store root is the **5-level** `users/<user>/zones/<zone>/workspaces/<workspace>/` (Z1–Z8 + the S4 guard, `395516d`..`fa2e37f`, each coder→reviewer→commit). A zone GROUPS a user's workspaces (e.g. `work` vs. `personal`) and is a **CONTENT ADDRESSING PREFIX, NOT a value-cascade rung** — the cascade stays framework→instance-global→workspace, with no `user` rung and no `zone` rung. **Selection:** `--zone` (CLI) / `zone` body field (HTTP shim, client library, n8n) defaults to `default` (single-zone users never type it), rides the same rails as `--user` at every door, and the RESOLVED zone is **ECHOED** back (result-envelope `zone` field + the friendly preview header + the client `echoed_zone`). **S4 spend guard:** on the CLI friendly door a SPEND verb (`preview`/`generate`/`outline drive`) run by a user who owns >1 zone REFUSES if `--zone` is omitted (lists the zones, exit 1, nothing spent) rather than silently pick `default`; single-zone users + all Tier-A verbs keep the friendly default, and an explicit `--zone` (even `default`) is always honored (the programmatic doors instead default a missing zone to `default`). **CRUD:** `zone new`/`list`/`delete` (delete safe-by-default, recursive two-tier, never through a symlink) + `--zone` on `workspace new`/`list`/`delete` and `entry`. **Same workspace name across zones is ALLOWED** — `users/dave/zones/{work,personal}/workspaces/acme` are DISTINCT, store-isolated workspaces (ids are store-scoped, resolved within one `(user, zone, workspace)` store; no global name index). **Identity-neutral:** the zone is in no id preimage, so relocating a workspace into a zone keeps every id — the migration is a pure relocation. **Migration:** `scripts/migrate-to-zones-layout.sh` (run ONCE, all-users, idempotent, never-destructive) in a fixed **pull → migrate → use** order (`update-from-upstream.sh` brings the zoned code + additive `.gitignore` before any data moves). **BREAKING (config):** the HTTP-shim allow-list key changed `user/workspace` → `user/zone/workspace` (+ `user/zone/*` + `user/*`) — a deployed `instance/shim.yaml` must migrate. **DEFERRED to the transport build (NOT delivered here):** the per-zone transport-credential rung (keys/weekly-caps assignable per zone) and therefore SPEND-BUCKET isolation across same-named zones — today the restructure gives **STORE isolation only**. Follow-up: GAP-13 (`recipe`/`select` still author into the default zone only). Design homes: design §10, §12.2, §23. Docs swept: design §10/§12/§23, `docs/guide/{interfaces,clients,concepts}.md`, `docs/{bootstrap,operating-model,claude-code-usage}.md`, `README.md`, `quickstart.md`; the CLAUDE.md rule-2 wording update is a maintainer action (prepared as a `.patch`, rule 5). |
| 0.3.1 | 2026-08-08 | **DIAGRAM TOOLS OPTIONAL** — graceful degradation + tool-less CI (`51a8735`; coder→reviewer→fix→independent-reverify). A requested `{type=diagram}` whose tool (graphviz `dot` / `d2` / `rsvg-convert`) is ABSENT now degrades gracefully — the diagram is dropped, its **text alt** substituted, and a warning is logged AND recorded (`ComposeOutcome.warnings` / `DispatchOutcome.warnings`), never a silent drop. **REVERSES the §17 PA-12 loud-fail for the MISSING-TOOL case ONLY** — the HARD grounding gate and malformed-source (`DiagramCompileError`) still refuse loudly (a missing tool is graceful; a lie or a broken spec is not). Identity stays honest: compose degradation stores no SVG (nothing folds into the serialize preimage); the docx/rsvg path makes the driver embed-fold **rsvg-aware** (`_rsvg_aware_embed_fold`) so the recorded digest matches the produced no-embed bytes — closing a same-id/different-bytes hazard (FR7.3). **CI now runs TOOL-LESS** (mirrors the common host that never installs graphviz), so a missing tool can no longer set CI red — proven by a full-suite tool-less run (3819 passed / 34 skipped / 0 failed); tool-full stays 3853 passed. Tests: generation tests skip when their tool is absent (shared `conftest` markers `requires_dot`/`requires_d2`/`requires_rsvg`); new tests pin the graceful path, the ungrounded-still-refused invariant, and the driver rsvg fold. Also pins `.python-version` 3.12.13 (fixes 3 unrelated `ipaddress.is_global` CI failures). Docs swept: the `diagram-styles` `tool` schema definition (+ regenerated `docs/reference/attributes.md`), a supersession note in `docs/known-issues.md`, and `docs/guide/concepts.md`. |
| 0.3.0 | 2026-08-06 | **SOURCES SUBSYSTEM** BUILT — pluggable acquire→cache→ground grounding sources behind the stable `Fact` contract (the narrow waist), governed by a benefit/cost θ + a **publish-vs-lead** gate; 7 increments (P0–P3b, `d9cf98f`..`6f016f2`), each research→design→adversarial→reconciliation then coder→reviewer→commit. **P0** the `reuse_rights`/`republishable` gate: `publishable`(=tier==EXTRACTED, §6.5 invariant, UNTOUCHED) AND a NEW `republishable` (a 5-value content-kind ordinal `forbidden<internal-only<lead-only<attribution<full`, schema floor `full` ⇒ byte-identical backward-compat) ANDed at `driver.py:781` — orthogonal to confidence, NOT selectable (no §5 dimension blur). **P1** a sealed content-addressed append-only slice CACHE + read-only `CacheReaderAdapter` ("cache"), the slice digest as the §7.2 commit-pin (never None ⇒ CF-1 + N2 by immutability) + the CLAUDE.md rule-1 CARVE-OUT (may cache acquired third-party content under the workspace, gitignored, never committed). **P2a** the HEAD-freeze (full N2 at the driver AND the production `session._generate_next` seam); GAP-12 Resolved. **P2b-core** the acquisition skeleton + `pipeline sources ingest` (out-of-band Tier-A) + **SEC EDGAR** — the first non-code source that PUBLISHES (host-pinned characterized primary → EXTRACTED + attribution) + `temporality` (archival|snapshot|live) as an identity-neutral §15-ledger provenance label. **P2b-feeds** `feedparser` (BSD-2) + RSS/Atom + GDELT (INFERRED leads; EXTRACTED stays EDGAR-class). **P3a** `trafilatura` (Apache-2.0) + Common Crawl lead-source + the pluggable `SearchBackend` (SearXNG deferred) + `THIRD_PARTY-LICENSES.md`. **P3b** the agentic RESEARCH loop — the first PAID acquisition — money-safe by construction (paid LLM seam injected ONLY from the CLI on `--go`; `pipeline/sources/**` imports no paid machinery; dry-run spends nothing; subscription-only F10 key-strip; disclosed `acquire-scope` is the true worst-case ceiling; θ-bounded). Runtime deps grew 1→3 (all permissive/MIT-compatible). Design homes: design §6 (sources & grounding), §6.5 (publish floor), §7.2 (identity), §21.9 (money-safety), §23; **design.md §sources canonical writeup deferred (design-of-record captured off-repo).** Deferred: SearXNG live backend; the paid-source keystore (shared with the transport-selection design); the anchor-depth ordinal. |
| 0.2.6 | 2026-07-24 | DR-1 (HTTP shim + poll/webhook async door for cloud orchestrators) BUILT — an additive, transport-agnostic HTTP shim over the existing `invoke()` (`pipeline/api/http_shim.py`, `scripts/pipeline serve`; no new dependency, no business logic): **Tier-A** cheap verbs served synchronously (the same `invoke()` JSON envelope + the N-4 HTTP-status mapping) + **Tier-B** paid verbs (`generate-next`, `render`) as **202-Accepted** with a client CHOICE of delivery — **poll** (always-available floor; a legitimately-running job is never told to resend — the job-lifetime window + "already-running ⇒ don't re-spawn" prevent double-charge) OR an optional **webhook** wakeup ping (approved-host allow-list + SSRF block, validated at submit AND re-validated at delivery, no redirects, bounded retry, never crashes the runner) + mandatory fail-closed bearer auth + loopback-default bind + workspace allow-list + DoS guards + an advisory concurrency cap and the `rate-limit-backpressure`→429 backstop. Prerequisites GAP-9 (workspace-root isolation) + GAP-10 (render mint-claim) landed standalone first. Commits `2f29e88`,`3e8d5b6`,`6f58881`,`444f046`..`b0c7088`. Design homes: design §21.7 (the "same JSON envelope" amendment admitting the 202-ack + poll status mapping as sanctioned additive wire shapes), §21.9 (the persistent-transport-front amendment, C-1). `begin-session{generate!=none}` (compose-in-one-call), Path B transport-chokepoint concurrency, webhook per-caller API keys, and connection-IP-pinning for the DNS-rebinding residual are deferred (`docs/known-issues.md` DR-1). |
| 0.2.5 | 2026-07-21 | DR-2 (lexicons/ house-style registry) BUILT — the compose-time `lexicons/` house-style registry (the 15th registry; attributes-only, entry body doc-only / never compose-consumed) + the resolved lexicon `{entry, delta}` as a top-level omit-when-absent 6th artifact-id preimage component + prompt-only compose application (byte-stable block) + the §11.2 additive-at-floor lint exemption (C3a, SV11 clause 1) + the L3 (workspace) + L5 (recipe) cascade wiring (L6 deferred, never-global) + the §16 reconcile ADVISORY preserve clause; commits `13dd6e6`..`782f3d0`. Design homes: design §4, §5.2, §6.5, §7.2, §11.2, §12.6, §16, §27.4. L6 selection-level + §12.6(b) lexicon-scope-default & brand-lock(b) are deferred, and schema-lint scans the 14-collection matrix excluding `lexicons/` (validated by `tests/test_lexicons.py`) (`docs/known-issues.md` DR-2). |
| 0.2.4 | 2026-07-20 | DR-5 (per-output citation style from one IR) BUILT — the top-level body-blind `references` CSL-JSON block on the IR (identity-neutral) + machinery-projected `references` from the grounding ledger's pool sources (`project_references`) + a writer that emits `[@key]` markers ONLY (no self-authored bibliography, #5) + the HARD `[@key]`->`references` resolution at compose (`citation-unresolved`) + `references` threaded into the AST `meta` frontmatter + content-driven citeproc enablement (the SD-5 identity twin) + the per-venue `csl` Presentation lever + the preserve-inline-`[@key]` reconcile obligation (`citation-not-preserved`) + the RI14 external-payload citeproc requirement block + the three-journals-one-paper (numeric / author-date / footnote) driving example; commits `0f9be37`..`08cff1f` (C9 deferred). Design homes: design §5.3, §6.5, §15, §16, §17, §19, §27.4. The C9/SF-6 output-type↔style compatibility WARN is deferred (`docs/known-issues.md` DR-5). |
| 0.2.3 | 2026-07-19 | DR-4 (content templates: typed-section conformance) BUILT — a typed-section conformance envelope over the outline's sections (DR-3 × DR-4 unified) — the F1 sentinel section grammar (`parse_sections`) + the OPEN `SECTION_TYPES` set + the pure conformance vocabulary (`check_conformance`, `Rule = Presence|Order|Count|Length`) + the Format `section_schema` attribute (academic-paper genre) + the omit-when-absent `section_conformance` IR field + the HARD base structural gate at compose + the per-venue reconcile structural gate (5th omit-when-floor `structural` preimage component) + the Platform `format_structural` HARD class + the preserve-through / no-mint role backstop + the SD-5 strip-invalid-keep-valid public-render transform + the three-venues-one-paper driving example; commits `b0ee351`..`c3d7311`. Design homes: design §5.2, §12.7, §15, §16, §17, §27.4. Nested-in-prose slots + whole-part sub-output conformance + content-KIND validators are deferred (`docs/known-issues.md` DR-4). |
