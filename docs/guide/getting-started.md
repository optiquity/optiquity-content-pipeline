...
3. **Create a workspace.** Each client gets an isolated workspace under your user prefix, made by copying the template: mkdir -p users/<user>/workspaces && cp -R templates/workspace users/<user>/workspaces/self (§23). The workspace name — and its owning user — are required arguments throughout the engine: for example, CascadeEnv.__init__ takes user and workspace parameters, and every CLI command that names --workspace also requires --user.
