...
Locally, the friendly way in is `scripts/pipeline` (also `uv run pipeline`) — English-ish flags onto the same verbs, built so nothing paid happens by accident (`generate` is a dry-run until you add `--go`, and spend runs on your Claude subscription by default — an API key is used only where you've explicitly assigned one, see [Transport](../transport.md)). Every command and its exact flags are catalogued in the generated [CLI reference](../reference/cli.md); the doors below are one line each:
