...
**The S4 spend guard (CLI friendly door).** A **spend** verb — `preview`, `generate`, and `outline drive` (whose `--go` actually spends) — invoked by a user who owns **more than one zone** REFUSES if `--zone` is omitted: it lists the available zones, exits 1, and spends nothing, rather than silently pick `default` and spend in the wrong zone. A single-zone user, and every (non-spending) Tier-A verb, keep the friendly `default`; an explicit `--zone` (even `--zone default`) is always honored. This guard lives at the CLI friendly spend door; the programmatic doors (HTTP shim, client library, n8n) instead default a missing zone to `default`, so a multi-zone caller on those doors must pass the `zone` explicitly.
